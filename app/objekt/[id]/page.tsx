...
export default function BillboardPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && billboard) {
      const favs = JSON.parse(localStorage.getItem('favorites') || '[]');
      setIsFavorite(favs.includes(billboard.id));
    }
  }, [billboard]);
  ...
  const toggleFavorite = () => {
    ...
    if (favs.includes(billboard.id)) {
      favs = favs.filter((id: string) => id !== billboard.id);
      setIsFavorite(false);
    } else {
      favs.push(billboard.id);
      setIsFavorite(true);
    }
              ...
              <button
                ...
                onClick={async () => {
                  const shareData = {
                    ...
                    text: `Kolla in denna reklamplats: ${billboard.title}`,
